...
let selectUserDefaultAddress = function(UserId){
    let _sql = 'select Id,Name,Phone,Address from users_address where IsDefault=1 and UserId='+UserId;
    return query(_sql);
}
// 更改默认地址
...
let findAllCarousel = function(){
    let _sql = "select c.Id,c.Title,c.Describe,c.Picture from Carousel c"
    return query(_sql);
}
...
const updateGoodsNumById= function(quantity,id){
...
// 旅行必备物品订单 start
const getGoodsOrderList = function(searchData){
    let _sql = "select * from goods_orders "
    if(searchData.globalName){
        _sql+= `where Out_Trade_No = '${searchData.globalName}' `
    }
    _sql+=` order by CreateTime desc limit ${(searchData.pageNo-1)*searchData.pageSize},${searchData.pageSize}`
    return query(_sql);
}
const getGoodsOrderLength = function(searchData){
    let _sql = "select count(1) count from goods_orders "
    if(searchData.globalName){
        _sql+= `where Out_Trade_No = '${searchData.globalName}' `
    }
    return query(_sql);
}
let insertGoodsOrder =function(value){
    let _sql =`insert into goods_orders(UserId,GoodsId,Name,Phone,Address,GoodsName,Photo,Price,Quantity,Seller_Id,Out_Trade_No,Trade_No,Total_Amount) values ${value}`
    return query(_sql);
}
let selectGoodsOrderByUserId =function(userId){
    let _sql ="select * from goods_orders where UserId = "+userId + ' order by CreateTime desc'
    return query(_sql);
}
// end 旅行必备物品订单
...
const insertStrategy= function(value){
    let _sql = "insert into strategy_info(Title,Pictures,Content,Address,UserId) values(?,?,?,?,?);"
    return query(_sql,value);
}
...
const selectStrategyByUserId = function(userId){
    let _sql = "select * from strategy_info where UserId = "+userId + ' order by CreateTime desc';
    return query(_sql);
}
...
const getAllTouristSpot = function(){
    let _sql = "select Id,Name from tourist_spots"
    return query(_sql);
}
...
const getTouristSpotTicketList = function(searchData){
    let _sql = "select tst.*,ts.Name,ts.Pictures from tourist_spots_ticket tst left join tourist_spots ts on ts.Id = tst.TouristSpotId"
    if(searchData.globalName){
        _sql+= ` where ts.Name = '${searchData.globalName}' `
    }
    _sql+=` order by CreateTime desc limit ${(searchData.pageNo-1)*searchData.pageSize},${searchData.pageSize}`
    return query(_sql);
}
const getTouristSpotTicketLength = function(searchData){
    let _sql = "select count(1) count from tourist_spots_ticket tst left join tourist_spots ts on ts.Id = tst.TouristSpotId"
    if(searchData.globalName){
        _sql+= ` where ts.Name = '${searchData.globalName}' `
    }
    return query(_sql);
...
const insertTouristSpotTicket= function(value){
    let _sql = "insert into tourist_spots_ticket(TouristSpotId,Type,Num,Price,Status) values(?,?,?,?,?);"
    return query(_sql,value);
}
const findTouristSpotTicketById= function(id){
    let _sql = "select tst.*,ts.Name,ts.Pictures from tourist_spots_ticket tst left join tourist_spots ts on ts.Id = tst.TouristSpotId where tst.Id = "+id;
    return query(_sql);
}
...
const updateTouristSpotTicket= function(value){
    let _sql = "update tourist_spots_ticket set TouristSpotId=?,Type=?,Num=?,Price=?,Status=?,AppointTime=? where Id=?;"
    return query(_sql,value);
}
...
const updateSpotTicketNumberById= function(id){
    let _sql = "update tourist_spots_ticket set Num = Num-1 where Id = "+id;
    return query(_sql);
}
// end 旅游景点门票

// 旅游景点登记信息 start
const insertTouristSpotTicketOrder= function(value){
    let _sql = "insert into tourist_spots_ticket_order(TouristSpotTicketId,Name,IdCardNumber,Phone,UserId) values(?,?,?,?,?);"
    return query(_sql,value);
}
const findTouristSpotTicketOrderCount= function(userId,ticketId){
    let _sql = `select count(1) count from tourist_spots_ticket_order where UserId = ${userId} and TouristSpotTicketId=${ticketId}`
    return query(_sql);
}
const selectTouristSpotByUserId= function(userId){
    let _sql = `select orders.Id,spots.Name SpotName,ticket.Type,spots.Pictures,ticket.Price from tourist_spots_ticket_order orders
    left join tourist_spots_ticket ticket on ticket.Id = orders.TouristSpotTicketId
    left join tourist_spots spots on spots.Id = ticket.TouristSpotId where orders.UserId = ${userId}` + ' order by orders.CreateTime desc'
    return query(_sql);
}
// end 旅游景点门票登记信息
...
const updateRoomUseById= function(isUse,id){
    let _sql = `update hotel_room set IsUse =${isUse} where Id = ${id}`;
    return query(_sql);
}
...
const selectHotelRoomAppoint= function(value){
    let _sql = "select * from hotel_room_appoint where RoomId = ? and UserId = ?;"
    return query(_sql,value);
}
const selectHotelRoomAppointById= function(id){
    let _sql = "select hra.*,hr.IsUse from hotel_room_appoint hra left join hotel_room hr on hr.Id = hra.RoomId where hra.Id = "+id;
    return query(_sql);
}
const insertHotelRoomAppoint= function(value){
    let _sql = "insert into hotel_room_appoint(RoomId,UserId,Name,IdCardNumber,Phone,AppointTime) values(?,?,?,?,?,?);"
    return query(_sql,value);
}
const deleteHotelRoomAppoint= function(value){
    let _sql = "delete from hotel_room_appoint where RoomId = ? and UserId = ?;"
    return query(_sql,value);
}
const deleteHotelRoomAppointById= function(id){
    let _sql = "delete from hotel_room_appoint where Id = "+id;
    return query(_sql);
}
const selectHotelRoomAppointByUserId= function(userId){
    let _sql = "select hra.Id,h.Name HotelName,hr.Pictures,hr.RoomCode,hr.BedType,hr.Price,hr.IsUse,hra.CreateTime,hr.HotelId from hotel_room_appoint hra left join hotel_room hr on hr.Id = hra.RoomId left join hotels h on h.Id = hr.HotelId where hra.UserId="+userId + ' order by hra.CreateTime desc';
    return query(_sql);
}
// end 预约房间

// 酒店订单 start
const insertHotelRoomOrder= function(value){
    let _sql = "insert into hotel_room_orders(RoomAppointId) values(?);";
    return query(_sql,value);
}
...
module.exports = {
    ...
    updateSpotTicketNumberById,
    // 旅游景点登记信息 start
    insertTouristSpotTicketOrder,
    findTouristSpotTicketOrderCount,
    selectTouristSpotByUserId,
    // 旅游景点评论
    ...
    selectHotelRoomAppointById,
    insertHotelRoomAppoint,
    deleteHotelRoomAppoint,
    selectHotelRoomAppointByUserId,
    deleteHotelRoomAppointById,
    // 社区服务
    getCommunityList,
    getCommunityLength,
    insertCommunity,
