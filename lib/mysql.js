...
let selectCommentInfo =function(globalName,pageNo,pageSize){
    let _sql =`select c.*,u.Username,u.Avatar from good_comments c left join Users u on c.UserId=u.Id where g.Name like '%${globalName}%' or u.Username like '%${globalName}%' limit ${pageNo},${pageSize}`
    return query(_sql);
}
let selectCommentInfoLength =function(globalName){
    let _sql =`select count(1) as count from good_comments c left join Users u on c.UserId=u.Id where g.Name like '%${globalName}%' or u.Username like '%${globalName}%'`
    return query(_sql);
}
...
const insertCommunity= function(value){
    let _sql = "insert into community(UserId,Content,PraiseNum) values(?,?,?);"
    return query(_sql,value);
}
...
const updateCommunity= function(value){
    let _sql = "update community set UserId=?,Content=?,PraiseNum=? where Id=?;"
    return query(_sql,value);
}
