...
let selectGoodsOrderByUserId =function(userId){
    let _sql ="select * from goods_orders where UserId = "+userId
    return query(_sql);
}
// end 旅行必备物品订单
...
const selectHotelRoomAppointByUserId= function(userId){
    let _sql = "select h.Name HotelName,hr.Pictures,hr.RoomCode,hr.BedType,hr.Price,hra.CreateTime from hotel_room_appoint hra left join hotels h on h.Id = hra.HotelId left join hotel_room hr on hr.Id = hra.RoomId where hra.UserId="+userId;
    return query(_sql);
}
// end 预约房间
